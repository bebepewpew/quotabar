...
Outside the structure `scripts/check-harness` gates, a defect here fails silently
in every future session: coverage never moves and nothing goes red, so a reader
is the only detector. Judge the change by the
...
structural drift `scripts/check-harness` catches before you read a word — is
