...
public final class UsageHistoryCoordinator: @unchecked Sendable {
    ...
    /// How far back `reload` reads. The advisor needs `Advisor.adviceLookback`;
    /// a caller that draws a longer strip than that needs its own span instead.
    var lookback: TimeInterval { max(Advisor.adviceLookback, span) }
    ...
    ///
    /// Blocking: it takes the history file's lock and reads back over
    /// `lookback`. Call it off the main actor.
    public func reload(recording successful: [QuotaSnapshot],
        ...
        let samples: [UsageSample]? = gate.withLock {
            ...
            // Only the span this pass can render: `span` for the strips, and
            ...
            // what the panel promises it does not do.
            return history.read(from: now.addingTimeInterval(-lookback), to: now).samples
        }
