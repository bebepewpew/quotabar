...
- `.claude/agents/` — one per role: `quotabar-pm`, `quotabar-product`,
  ...
  `quotabar-ciso` and `quotabar-release`. Each is usable on its own; the workflows only decide
...
- `.codex/skills/` — the same roles with `agents/openai.yaml` manifests. Codex
  ...
  in `.claude/agents/` and `.codex/skills/`. Codex reads `AGENTS.md` natively, which is why
